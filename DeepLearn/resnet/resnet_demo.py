...
train_iter = iter(tensor_train)
test_iter = iter(tensor_test)
...
class ResnetBlock(torch.nn.Module):
    ...
    def forward(self, X):
        ...
        #判断是否对输入进行1x1卷积
        if self.stride == 2:
            X = nn.Conv2d(self.input_features, self.output_features, 1, self.stride, 0)(X)
        hid += X
        #对最后的结果进行激活
        return self.act(hid)
...
writers.add_graph(my_net, [next(train_iter)[0], ])
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        for xi, yi in test_iter:
            ...
            ac += torch.sum(torch.argmax(y_hat, dim=1) == yi)
            count += len(xi.shape[0])
        ac /= count
        tmp_loss /= count

        writers.add_scalars('train_clue', {'loss': tmp_loss, 'ac': ac}, global_step=epoch)
